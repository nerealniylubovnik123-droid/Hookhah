// server.js — fixed for Render deploy (no self-referencing 'app' during init)
...
// Serve static files if you deploy the frontend together (optional)
...
// Create files if missing (first deploy)
...
if (!fs.existsSync(BANNED_FILE))  writeJSON(BANNED_FILE, [
  "спайс", "наркотик", "взрывчатка", "18+", "xxx"
]);

// Helpers
const normalize = (s) => String(s || "").toLowerCase().replace(/ё/g, "е");
const uniq = (arr) => Array.from(new Set(arr));
function findBannedInText(text, list) {
  const t = normalize(text);
  const hits = [];
  for (const w of list || []) {
    const ww = normalize(w);
    if (ww && t.includes(ww)) hits.push(w);
  }
  return uniq(hits);
}
...
app.get("/api/banned-words", (req, res) => {
  res.json({ words: readJSON(BANNED_FILE, []) });
});
...
app.post("/api/flavors", (req, res) => {
  ...
  const body = req.body || {};
  // support deletion via action:"delete" to match frontend usage
  ...
  const flavors = readJSON(FLAVORS_FILE, []);
  flavor.id = flavor.id || (String(flavor.brand) + "-" + String(flavor.name)).toLowerCase().replace(/\s+/g, "-");
...
app.post("/api/mixes", (req, res) => {
  const body = req.body || {};
  const bannedList = readJSON(BANNED_FILE, []);
  const hits = uniq([
    ...findBannedInText(body.title, bannedList),
    ...findBannedInText(body.notes, bannedList)
  ]);
  if (hits.length) {
    return res.status(400).json({ error: "banned_words", message: "Запрещённые слова", banned: hits });
  }
  const mixes = readJSON(MIXES_FILE, []);
